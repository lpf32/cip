...
static int tcp_transmit_skb(struct sock *sk, struct sk_buff *skb, uint32_t seq)
{
    ...
    // FIXME tcp->snd_una should be seq
    skb->seq = tcb->snd_una;
    skb->end_seq = tcb->snd_una + skb->dlen;
    // FIXME: assign correct value to skb->len
...
static int tcp_send_syn(struct sock *sk)
{
    if (sk->state != TCP_SYN_SENT && sk->state != TCP_CLOSE && sk->state != TCP_LISTEN) {
        print_err("Socket was not int correct state (closed or listen)\n");
    }
...
static void tcp_retransmission_timeout(uint32_t ts, void *arg)
{
    ...
    tsk->retransimt = timer_add(500, &tcp_retransmission_timeout, sk);
...
void tcp_rearm_rto_timer(struct tcp_sock *tsk)
{
    ...
    if (sk->state == TCP_SYN_SENT) {
        tsk->retransimt = timer_add(TCP_SYN_BACKOFF << tsk->backoff, &tcp_connect_rto, tsk);
    } else {
        tsk->retransimt = timer_add(500, &tcp_retransmission_timeout, tsk);
    }
}
...
int tcp_connect(struct sock *sk)
{
    ...
    tcp_select_initial_window(&tsk->tcb.snd_wnd);
