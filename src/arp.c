...
static struct arp_cache_entry *arp_entry_alloc(struct arp_hdr *hdr,
...
static int update_arp_translation_table(struct arp_hdr *hdr, struct arp_ipv4 *data)
{
    ...
    for (item = (&arp_cache)->next; item != &arp_cache; item = item->next) {
...
void arp_rcv(struct sk_buff *skb)
{
    ...
    if (arphdr->hwtype != ARP_ETHERNET) {
        print_err("ARP: Unsuppored HW type\n");
        goto drop_pkt;
    }
...
int arp_request(uint32_t sip, uint32_t dip, struct netdev *netdev)
{
    ...
    payload->dip = dip; // check ip whether or not request device
