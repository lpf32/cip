...
int tcp_v4_checksum(struct sk_buff *skb, uint32_t saddr, uint32 daddr)
{
    return tcp_udp_checksum(saddr, daddr, IP_TCP, skb->data, skb->len);
}

// FIXME: add socket proto
...
int tcp_v4_connect(struct sock *sk, const struct sockaddr *addr,
                   int addrlen, int flags)
{
    ...
    // FIXME: Do not hardcode lcok interface
...
int tcp_read(struct sock *sk, void *buf, int len)
{
    ...
    switch (sk->state) {
        ...
        case TCP_SYN_RECEIVED:
             ...
             * is no room to queue this request, repond with "error:
             * insufficient resource". */
        case TCP_ESTABLISHED:
        case TCP_FIN_WAIT_1:
        case TCP_FIN_WAIT_2:
            break;
        /* If no text is awaiting deliver, the RECEIVE will get a
...
int tcp_close(struct sock *sk)
{
    switch (sk->state) {
        ...
        case TCP_ESTABLISHED:
             ...
             * from a FIN segment and sent it. In any case, enter FIN-WAIT-1
             * state */
