...
/*
 * Acks all segments from retransmission queue that are "older"
...
static int tcp_verify_segment(struct tcp_sock *tsk,
                              struct tcphdr *th,
                              struct sk_buff *skb)
{
    ...
    if ((th->seq < tcb->rcv_nxt) || (th->seq > (tcb->rcv_nxt+ tcb->rcv_wnd))) {
        tcpsock_dbg("Received invaild segment", (&tsk->sk));
        return 0;
    }
...
static inline int tcp_discard(struct tcp_sock *sk,
                              struct sk_buff *skb,
                              struct tcphdr *th)
{
    free_skb(skb);
...
static int tcp_synsent(struct tcp_sock *tsk, struct sk_buff *skb, struct tcphdr *th)
{
    ...
    if (th->ack) {
        if (th->ack_seq <= tcb->iss || th->ack_seq > tcb->snd_nxt) {
            tcpsock_dbg("ACK is unacceptabler", sk);
    ...
    if (th->ack) {
        ...
        /* Any pakcets in RTO queue that are acknowledged here should be removed */
...
static int tcp_closed(struct tcp_sock *tsk, struct sk_buff *skb, struct tcphdr *th)
{
     ...
     * containing a RST causes a RST to be sent in response. The acknowledgment
     * and sequence field values are selected to make the reset sequence acceptable
     * to the TCP that sent the offending segment.
     *
     * If the ACK bit is off, sequence number zero is used,
     * <SEQ=0><ACK=SEG.SEQ+SEG.LEN><CTL=RST,ACK>
     *
     * If the akc bit is on,
     *
     * <SEQ=SEG.ACK><CTL=RST>
     * */
    int rc = -1;
...
int tcp_input_state(struct sock *sk, struct tcphdr *th, struct sk_buff *skb) {
    ...
    switch (sk->state) {
        ...
        case TCP_LAST_ACK:
            ...
            if (th->ack_seq > tcb->snd_nxt) {
                // If the ACK acs something not yet sent, then send an ACK, drop
                // segment and return
                // TODO: why would I respond with an ACK?
                return tcp_drop(tsk, skb);
            }
    ...
    if (th->fin && (tcb->rcv_nxt - skb->dlen) == skb->seq)
    {
        ...
        switch (sk->state) {
            ...
            case TCP_FIN_WAIT_2:
                /* Enter the TIME_WAIT state. Start the time-wait timer, turn
